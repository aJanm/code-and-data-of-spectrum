...
class Pretreatment:
    ...
    def PlotSpectrum(self, spec, title='原始光谱', x=0, m=5):
        ...
        if isinstance(spec, pd.DataFrame):
            spec = spec.values
        # 2023年2月19日
        print("spec.shape[1]:", spec.shape[1])
    ...
    def PlotSpectrum2(self, spec, title='原始光谱', x=0, m=5):
        """
        :param spec: shape (n_samples, n_features)
        :return: plt
        """
        ...
        print("spec.shape[1]:", spec.shape[1])
        legend_list = ['10mg/l', '20mg/l', '30mg/l', '40mg/l', 
'50mg/l', '60mg/l','70mg/l','80mg/l','90mg/l', '100mg/l', '水样4', 'ctest']
        # 定义原始raw_spec表示备份
        raw_spec = spec
        spec = spec[:, :(spec.shape[1]-1)]
        plt.rcParams['font.sans-serif'] = ['SimHei']  # 用来正常显示中文标签
        ...
        plt.xlabel("波长/nm", font)
        plt.ylabel("吸光度", font)
        # 因为里面就刚好有10~100间隔为10的对应总共10个光谱
        for i in range(10):
            y = raw_spec[i, :]
            if i == 0:
                plt.plot(x, y, label=legend_list[i], linestyle='-')
            elif i == 9:
                plt.plot(x, y, label=legend_list[i], linestyle='-')
            elif i == 10:
                plt.plot(x, y, color="black", label=legend_list[i], linestyle='-.')
            elif i % 3 == 0:
                plt.plot(x, y, label=legend_list[i], linestyle=(0, (3,5,1,5)))
            elif i % 3 == 1:
                plt.plot(x, y, label=legend_list[i], linestyle='-.')
            elif i % 3 == 2:
                plt.plot(x, y, label=legend_list[i], linestyle=(0, (5, 5))) # 间距较大的两段虚线
            elif i % 3 == 0:
                plt.plot(x, y, label=legend_list[i], linestyle=(0, (5,10)))
            plt.legend(loc=0)
        # plt.plot(x, raw_spec[0, :])
        plt.title(title, font)
        name = self.getDateStr(0)+title
        file_name = "./graph_result/" + self.getDateStr(0)
        if os.path.exists(file_name):
            print(file_name+"文件夹已存在")
        else:
            os.mkdir(file_name)
        # print("name:", name)
        # plt.savefig('./2023年2月20日_test1.svg', bbox_inches='tight')
        plt.savefig(file_name+name+'.svg', bbox_inches='tight')
        # fonts = 6
        # plt.figure(figsize=(5.2, 3.1), dpi=200)
        # plt.plot(wl, spec.T)
        # plt.xlabel('Wavelength (nm)', fontsize=fonts)
        # plt.ylabel('reabsorbance', fontsize=fonts)
        # plt.title(title, fontsize=fonts)
        # plt.savefig('./2023年2月20日_test2.svg', bbox_inches='tight')

        return plt
    
    # 英文版保存图片，2023年3月30日
    def PlotSpectrum2_english(self, spec, title='原始光谱', x=0, m=5):
        """
        :param spec: shape (n_samples, n_features)
        :return: plt
        """
...
'50mg/l', '60mg/l','70mg/l','80mg/l','90mg/l', '100mg/l', '水样4', 'ctest']
    ...
    def PlotSpectrum3(self, spec, title='原始光谱', x=0, m=5):  
        return
    ...
    def standardlize(self, sdata):
        ...
        """
        sdata = deepcopy(sdata)
        # isinstance用来判断一个对象的变量类型。
        # 如果是DataFrame类型则获取其数据
    ...
    def msc(self, sdata):
        ...
        n = sdata.shape[0]  # 样本数量，对应为50行
        k = np.zeros(sdata.shape[0])
        b = np.zeros(sdata.shape[0])
        print("n:",n," , k:", k)
        # shape[1]对应为列，也就是波长的宽度为600
        # axis=0表示对对纵向求平均值
        # 举例，以wl1为例，第一个就是求波长wl1这一列的均值
        # 也就是说M是数据部分中每列的均值
        # 下面的y对应第i行的所有数据列
        M = np.array(np.mean(sdata, axis=0))
        print("M.shpe:", M.shape) # M.shpe: (600,)
        from sklearn.linear_model import LinearRegression
        
        for i in range(n):
            y = sdata[i, :] # 取每一行的全部列 
            print("y.shape:",y.shape) # y.shape: (600,)
            # y.shape: (600,)
            y = y.reshape(-1, 1) # 将其转置为列
            print("y.reshape:", y.shape)# y.reshape: (600, 1)
            # print("y:", y)
            M = M.reshape(-1, 1)
            # M.shape: (600, 1)
